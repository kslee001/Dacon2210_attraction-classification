...
# fin_synlist = []
# synset_list = pd.read_csv("kwn_synset_list.tsv", sep = "\t", on_bad_lines="skip").dropna()
# synlist = synset_list["korean_lemmas"].str.split(", ").tolist()
# for l in synlist:
#     if len(l) >=3: fin_synlist.append(l)
...
def load_data(do_augmentation=False, do_embedding=False):
    ...
    def execute_augmentation(data:pd.DataFrame):
        seed_everything(CFG['SEED'])
        print("execute data augmentation...(it takes about 10 minutes to complete)")
        # for counting
        ...
        for a in (range(len(targets))):
            for cat_name in (targets[a]):
                ...
                while(cur_idx <= needed):
                    new_random_idx +=1
                    # image augmentation : sampling
                    random.seed(CFG["SEED"] + new_random_idx * 7) # random + random
                    ...
                    cur_txt = cur_df.loc[i]["overview_aug"]                    
    ...
    def execute_cnt_embedding(data:pd.DataFrame, augmented_data, max_features=CFG["embedding_dim"]):
        ...
        print("tokenizing...(it takes about 10 minutes to complete.)")
        for i in (range(len(texts))):
            tokenized = []
            for token in tokenizer.pos(texts[i], norm=True, stem=True):
                if (token[1] == "Noun") | (token[1] =="Adjective"):
                    tokenized.append(token[0])
            texts[i] = " ".join(tokenized)        
    ...
    # load pre augmented txt data
    pre_augmented_txt = pd.read_csv(data_path + "augmented_txt.csv")[["overview_aug"]]
    data = pd.concat([data, pre_augmented_txt], 1)
    ...
    for i in range(len(augmented_data["overview"])):    
        augmented_data.loc[i]["overview"] = re.sub(r"[^\uAC00-\uD7A30-9a-zA-Z\s]", "", augmented_data.loc[i]["overview"])
    ...
    return image_paths, texts_cntvector, y1, y2, y3
...
def generate_dataloader(X1:list, X2:torch.tensor, y1:list, y2:list, y3:list):
    seed_everything(CFG["SEED"])
