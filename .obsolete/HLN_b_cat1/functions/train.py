...
def train(model, optimizer, criterion, warm_up, scheduler, train_loader, val_loader, device=CFG["device"]):
    ...
    for epoch in range(1, CFG["EPOCHS"]+1):
        ...
        for x_img, x_text, y in (iter(train_loader)):
            ...
            yhat1 = model(img, text)
            loss = criterion(yhat1, y1)
            loss.backward()
            ...
            superclass_accuracy.append(calculate_accuracy(predictions=yhat1.detach(), labels=y1))
        ...
        print(f'Train acc : {supperclass_acc:.5f}')
        print(f'Valid acc : {val_supperclass_acc:.5f}')
...
def validation(model, criterion, val_loader, device=CFG["device"]):
    ...
    with torch.no_grad():
        for x_img, x_text, y in (iter(val_loader)):
            ...
            model_preds += yhat1.argmax(1).detach().cpu().numpy().tolist()
