...
def train(model, optimizer, criterion, warm_up, scheduler, train_loader, val_loader, device=CFG["device"]):
    ...
    for epoch in range(1, CFG["EPOCHS"]+1):
        ...
        for x_img, x_text_cntvector, y in (iter(train_loader)):
            img  = x_img.float().cuda()
            text_cntvector = x_text_cntvector.cuda()
            y1   = y[:, 0].cuda()
            y2   = y[:, 1].cuda()
            y3   = y[:, 2].cuda() 
            # ygt  = [y1, y2, y3]
            ...
            # yhat1, yhat2, yhat3 = model(img, text_cntvector)
            yhat1 = model(img, text_cntvector)
            
            # pred = [yhat1, yhat2, yhat3]
            pred = [yhat1]
            
            total_loss = criterion(pred[0], ygt)  
            total_loss.backward()
        ...
        val_loss, val_supperclass_acc, val_score = validation(model, criterion, val_loader)
...
def validation(model, criterion, val_loader, device=CFG["device"]):
    ...
    # subclass2_accuracy = []
    with torch.no_grad():
        for x_img, x_text_cntvector, y in (iter(val_loader)):
            img  = x_img.float().cuda()
            text_cntvector = x_text_cntvector.cuda()
            y1   = y[:, 0].cuda()
            y2   = y[:, 1].cuda()
            ...
            model_preds += pred[0].argmax(1).detach().cpu().numpy().tolist()
            true_labels += y1.cpu().numpy().tolist()
    ...
    return np.mean(val_loss), supperclass_acc, test_weighted_f1
