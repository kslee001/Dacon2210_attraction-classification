...
def train(model, optimizers:list, criterions:list, warm_ups:list, schedulers:list, train_loader, val_loader, device=CFG["device"]):
    ...
    for epoch in range(1, CFG["EPOCHS"]+1):
        ...
        if(epoch < 6):
            ...
            schedulers[1].step(val_loss1)   # for cat 1
            schedulers[2].step(val_loss2)   # for cat 2
            schedulers[3].step(val_loss3)   # for cat 3
...
def save_model(best_model):
    # save model
    # if configs.general["jit_flag"] :
    #     m = torch.jit.script(configs.best_model)
    #     with torch.no_grad():
    #         torch.jit.save(
    #             m,
    #             configs.general["output_folder"] + "/" + configs.general["model_name"] +"_jit"
    #         )
    #     return
    # else:
    ...
    torch.save(
        best_model, 
        CFG["output_folder"] + "/" + CFG["model_name"]
    )
    torch.save(
        best_model.state_dict(),
        CFG["output_folder"] + "/" + CFG["model_states_name"]
    )


def save_configs():
    # save configs
    with open(CFG["output_folder"] + "/" + "configs.txt", "w") as f:
