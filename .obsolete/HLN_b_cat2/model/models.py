...
class HLNCat2(torch.nn.Module):
    def __init__(self, num_classes=[6,18,128]):
        ...
        self.img_branch = LoadEffnetv2(m_size="m", m_type="branch", 
                                        num_classes=num_classes[1]*self.image_weight, origin_input_channels=3)
        
        """text classifier"""
        self.txt_main = torch.nn.Linear(768, num_classes[1]*self.text_weight)

        """final classifier"""
        self.fin_classifier1 = torch.nn.Linear(
            num_classes[1]*(self.image_weight + self.text_weight), 
            num_classes[1]*(self.image_weight + self.text_weight)//2
        )
