    ...
    for i in range(len(targets)):
        for cat_name in targets[i]:
            ...
            while(cur_idx <=needed):
                new_random_idx +=1
                # sampling
                txt_aug_seed = random.seed(CFG["SEED"] + new_random_idx) # new random seed
                sample_idx = random.sample(img_index, 1)[0]
                
                # labels
                cur_cat1 = cur_df.loc[sample_idx]["cat1"]
                cur_cat2 = cur_df.loc[sample_idx]["cat2"]
                cur_cat3 = cur_df.loc[sample_idx]["cat3"]
                augmented_cat1.append(cur_cat1)
                augmented_cat2.append(cur_cat2)
                augmented_cat3.append(cur_cat3)
                
                # img augmentation
                path    = cur_df.loc[sample_idx]["img_path"]
                ...
                # text augmentation
                cur_txt    = cur_df.loc[sample_idx]["overview"]
                candidates = eda.EDA(cur_txt, random_state=txt_aug_seed)
                cur_txt    = random.choice(candidates)
                augmented_text.append(cur_txt)
                
                cur_idx +=1
                
    # step 2. 한식, 야영지 제외 전체 이미지 텍스트 augmentation
