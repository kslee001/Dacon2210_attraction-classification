...
def train(model, optimizer, con_optimizer, criterion, warm_up, scheduler, con_scheduler, train_loader, val_loader, device=CFG["device"]):
    best_score = 0
    best_model = None
    
    for epoch in range(1, CFG["EPOCHS"]*2 + 1): # init train CFG["EPOCHS"] + continue train CFG["EPOCHS"]
        model.train()
        train_loss = []
