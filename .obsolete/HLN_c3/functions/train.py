...
def train(model, optimizer, criterion, warm_up, scheduler, train_loader, val_loader, device=CFG["device"]):
    ...
    for epoch in range(1, CFG["EPOCHS"]+1):
        ...
        for x_img, x_text_cntvector, x_text_raw, y in (iter(train_loader)):
            ...
            yhat1, yhat2, yhat3 = model(img, text_cntvector, text_raw)
...
def validation(model, criterion, val_loader, device=CFG["device"]):
    ...
    subclass2_accuracy = []
    with torch.no_grad():
        for x_img, x_text_cntvector, x_text_raw, y in (iter(val_loader)):
            img  = x_img.float().cuda()
            text_cntvector = x_text_cntvector.cuda()
            text_raw       = x_text_raw.cuda()
            y1   = y[:, 0].cuda()
            y2   = y[:, 1].cuda()
            y3   = y[:, 2].cuda() 
            ygt  = [y1, y2, y3]
