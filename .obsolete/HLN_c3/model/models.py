...
class HLNc3(torch.nn.Module):
    def __init__(self, num_classes=[6,18,128]):
        ...
        self.txt1_cntvector_branch = torch.nn.Linear(2048, 128)
        self.txt2_cntvector_branch = torch.nn.Linear(2048, 256)
        self.txt3_cntvector_branch = torch.nn.Linear(2048, 512)
        
        # sentence embedding
        self.txt_sentence_main = torch.nn.Linear(768, CFG["sentence_dim"])
        self.txt1_sentence_branch = torch.nn.Linear(CFG["sentence_dim"], 128)
        self.txt2_sentence_branch = torch.nn.Linear(CFG["sentence_dim"], 256)
        self.txt3_sentence_branch = torch.nn.Linear(CFG["sentence_dim"], 512)
        

        """final classifier"""
        self.linear_lvl1 = torch.nn.Linear(1024+128+128, num_classes[0])
        self.linear_lvl2 = torch.nn.Linear(2048+256+256, num_classes[1])
        self.linear_lvl3 = torch.nn.Linear(8192+512+512, num_classes[2])
    ...
    def forward(self, x_img, x_txt_cntvector, x_txt_raw):
        """image stage"""
        ...
        """text stage 1 (word count vector)"""        
        ...
        """text stage 2 (sentence vector)"""
        # main network for sentence embedding    
        x_txt_raw = self.txt_sentence_main(x_txt_raw)        
        # branch networks
        txt_stc_level_1 = self.txt1_sentence_branch(x_txt_raw)
        txt_stc_level_1 = self.activate(txt_stc_level_1)
        
        txt_stc_level_2 = self.txt2_sentence_branch(x_txt_raw)
        txt_stc_level_2 = self.activate(txt_stc_level_2)
        
        txt_stc_level_3 = self.txt3_sentence_branch(x_txt_raw)
        txt_stc_level_3 = self.activate(txt_stc_level_3)
                
        """final classification stage"""
        cat1 = torch.cat((img_level_1, txt_cnt_level_1, txt_stc_level_1), dim=1)
        cat2 = torch.cat((img_level_2, txt_cnt_level_2, txt_stc_level_2), dim=1)
        cat3 = torch.cat((img_level_3, txt_cnt_level_3, txt_stc_level_3), dim=1)
