...
def ori_transform(x:np.array):
    ...
    return transforms.Normalize(mean=mean,std=std)(tf_sequence(image=x)["image"].float())


def con_transform(x:np.array):
    mean = (0.5071, 0.4867, 0.4408)
    std = (0.2675, 0.2565, 0.2761)
    
    seed_everything(CFG['SEED'] + seed_number) # new random seed
    # soft transformation
    # x = x.permute(0,2,1).numpy()
    tf_sequence = A.Compose([
        A.RandomCrop(CFG["IMG_SIZE"], CFG["IMG_SIZE"]),
        A.OneOf([
            A.RandomBrightness(limit=20, p=1)
        ]),
        A.OneOf([
            A.RandomGamma(p=1),
            A.RandomRain(p=1),
...
def execute_img_read(image:str):
...
class AugmentDataset(Dataset):   # X1 : img path  / X2 : txt vector / X3 : txt raw
    ...
    def __getitem__(self, idx):
        ...
        if(data_type =="jpg"):
            image = execute_img_read(image)
        elif(data_type =="pt"):
            image = self.augmented_img[int(self.X_images[idx].split(".pt")[0])]

        # generate augmented image (2) for contrastive learning
        # image
        
        ori_image = ori_transform(image)
        con_image = con_transform(image)
        image = [ori_image, con_image]

        # text
        text  = self.X_texts[idx]
        augmented_txt = eda.EDA(text)
        augmented_txt = random.choice(augmented_txt)
        ...
        ori_text_input_ids = text["input_ids"].squeeze(0)
        ori_text_attention_mask = text["attention_mask"].squeeze(0)
        
        augmented_txt = self.tokenizer.encode_plus(
            augmented_txt,
            add_special_tokens=True,
            max_length=256,
            return_token_type_ids=False,
        ...
        augmented_txt_input_ids = augmented_txt["input_ids"].squeeze(0)
        augmented_txt_attention_mask = augmented_txt["attention_mask"].squeeze(0)
        
        text_input_ids = [ori_text_input_ids, augmented_txt_input_ids]
        text_attention_mask = [ori_text_attention_mask, augmented_txt_attention_mask]
        ...
        if self.infer_flag :
            ...
            # note that the number of returning values are "list" (contains 2 values) !
