...
from config import *
from model.lossfn import HierarchicalLossNetwork, SupConLoss
...
if __name__ == "__main__":
    ...
    # define model
    model     = EffKlueBaseAugCon().cuda()
    optimizer = torch.optim.Adam(
        params       = model.parameters(), 
        lr           = CFG["LEARNING_RATE"],
    )
    criterion = [SupConLoss().cuda() for _ in range(3)]
