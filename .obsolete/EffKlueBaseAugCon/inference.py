from config import *
from model.lossfn import HierarchicalLossNetwork
...
def inference(model, test_loader, encoder, device):
    model.to(device)
    model.eval()
...
def main():
    ...
    i = 0
    model = EffKlueBase().cuda()
    model = torch.nn.DataParallel(model, device_ids = [0,1])
    model.load_state_dict(torch.load(inference_config["weight_dir"]))
    model.eval()
    model_preds = inference(model, test_loader, enc, device="cuda")
    del model
